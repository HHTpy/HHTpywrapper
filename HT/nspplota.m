%function [nt,tscale,fscale]=nspplota(f,a,t0,t1,fres,tres,fw0,fw1,tw0,tw1,lscale)
%
% Calling sequence-
% [nt,tscale,fscale]= nspplota(f,a[,t0][,t1][,fres][,tres][,fw0][,fw1][,tw0][,tw1][,lscale])
% Example, [nt,tscale,fscale]=nspplota(f,a,1,1000,500,300,0,1,1,1000).
...
%
% NOTE
% NSPPLOTA.m compute the Hilbert-Huang Spectrum of amplitude.
% The function NSPPLOTA generates and plots the HHT amplitude spectrum of data (or log data) 
...
%                 It is called by nnspe to compute the Hilbert-Huang spectrum of amplitude.
...
%because this calculate amplitude ,so AM not square! 
% the following loop checking FM about its position
% when the position is correct ,put its corresponding amplitude in that position.
for x=sidx:eidx                                       %checking every FM values-loop A start  
    for imf=1:nimf                                     %checking the FM values for every IMF-loop B start 
        ...
        if (freqidx >= 1 & freqidx <= fres)             %checking the position is 'inside' or 'outside' the grid 
            ...
            nt(tx,freqidx)=nt(tx,freqidx)+a(x,imf);     %put amplitude(AM) in its position
        end
